...
import { CONTRACT_CODES, CONTRACT_LABELS, type Round, type ScoringSettings } from '@shared/types';
...
export function AdminPage() {
  ...
  return <Shell>
    <header className="topbar"><div><span className="eyebrow">Administration</span><h1>{game.name || 'Partie de Tafaron'}</h1></div><nav><Link to="/game">Écran de jeu</Link><Link to="/tables">Tableaux</Link><Link to="/history">Historique</Link></nav></header>
    ...
    {(game.status === 'playing' || game.status === 'finished') && <main className="admin-grid">
      <section className="panel hero-panel"><div className="section-head"><div><span className="eyebrow">Manche {current?.index ?? '—'}</span><h2>{game.status === 'finished' ? 'Partie terminée' : `${chooser?.name ?? '—'} choisit`}</h2></div><StatusPill tone={game.status === 'finished' ? 'good' : 'warn'}>{game.status}</StatusPill></div>
        {game.status === 'playing' && current?.status === 'choosing' && <ContractPicker options={game.settings.enabledContracts} available={game.settings.enabledContracts.filter((code) => !game.rounds.some((r) => r.status === 'validated' && r.chooserPlayerId === current.chooserPlayerId && r.contract === code))} onChoose={(contract, jokerContract) => mutate('/round/contract', 'POST', { playerId: current.chooserPlayerId, contract, jokerContract })} />}
      ...
      <aside className="stack"><ScoreSummary snapshot={snapshot!} /><AdminActions game={game} current={current} mutate={mutate} onHome={async () => { if (await mutate('/archive')) navigate('/admin?new=1'); }} /></aside>
...
function ScoreSummary({ snapshot }: { snapshot: GameSnapshot }) { const ranking = [...snapshot.game.players].sort((a, b) => snapshot.scores[a.id] - snapshot.scores[b.id]); return <section className="panel"><span className="eyebrow">Classement actuel</span><h3>{snapshot.remainingRounds} manches restantes</h3><ol className="ranking">{ranking.map((player) => <li key={player.id}><span>{player.name}</span><strong>{snapshot.scores[player.id]} pts</strong></li>)}</ol></section>; }

function AdminActions({ game, current, mutate, onHome }: { game: GameSnapshot['game']; current?: Round; mutate: (path: string, method?: string, body?: unknown) => Promise<boolean>; onHome: () => void }) {
  return <section className="panel"><span className="eyebrow">Actions</span><div className="action-list"><a className="button" href={`/api/games/${game.id}/export/json`}>Exporter JSON</a><a className="button" href={`/api/games/${game.id}/export/rounds.csv`}>CSV manches</a><a className="button" href={`/api/games/${game.id}/export/scores.csv`}>CSV scores</a>{game.status === 'playing' && current?.status === 'choosing' && <details><summary>Paramètres entre les manches</summary><label>Mode de saisie<select value={game.settings.resultEntryMode} onChange={(e) => mutate('/settings', 'PATCH', { resultEntryMode: e.target.value })}><option value="players">Téléphones</option><option value="organizer">Admin</option></select></label><ScoringOptions scoring={game.settings.scoring} mutate={mutate} /></details>}{game.status === 'finished' ? <button className="primary" onClick={onHome}>Accueil</button> : <button className="danger ghost" onClick={() => confirm('Terminer la partie maintenant ?') && mutate('/finish')}>Terminer plus tôt</button>}<button className="danger" onClick={() => confirm('Annuler la dernière manche validée ?') && mutate('/rounds/undo')}>Annuler la dernière manche</button></div></section>;
}
